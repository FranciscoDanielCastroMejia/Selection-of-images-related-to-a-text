...
def load_pesos():
    ...
    return pesos_rb_cls, pesos_rb_mp, pesos_st_cls, pesos_st_mp
#__________________________________________________________________________________
#__________________________________________________________________________________   
#__________________________________________________________________________________
#__________________________________________________________________________________
...
#____importaremos la base de datos donde es encuentran los articulos con su resumen correspondiente

def look_article_img(text):
    with open('BD Dataset Articles/links_article_TS.json', 'r') as file:
        art = json.load(file)

    #invertimos los links (llaves) por el texto del articulo (valores) del diccionario de articulos para que la busqueda sea mas eficiente, sabiendo qeu los articulos no se repiten
    ...
    #esta prueba solo sirve si el texto que ingresamos es el de un articulo.
    print('\n')
    if text in articles:
        link_article = articles[text]
        print(f'Link de la imagen del articulo: {link_article}')
    else:
        link_article = 'Article not found'
        print(link_article)
...
def count_repeated_img(pesos_rb_cls, pesos_rb_mp, pesos_st_cls, pesos_st_mp):
    ...
    img_rep_and_not_rep = {} #diccionario donde se guardan las imagenes que se repiten y las que no se repiten
    ...
    return img_repetitions

#__________________________________________________________________________________
#__________________________________________________________________________________   
#__________________________________________________________________________________
#___________________________DISPLAY THE IMAGES________________________________
#__________________________________________________________________________________
def display_images(img_repetitions, link_article):
    ...
    for i, link in enumerate(top_img):
        ...
        # Agregar título si es la imagen del artículo
        if link == link_article:
...
if __name__== '__main__':
    ...
    text = "The first nuclear explosion in history took place in the New Mexico desert on July 16, 1945. A group of teenagers who were camping near Ruidoso, New Mexico, were awakened by an explosion. Hours after the explosion began to fall white flakes from the sky. The girls, only 13 years old, thought that this was because of the heat it did."
    ...
    pesos_rb_cls, pesos_rb_mp, pesos_st_cls, pesos_st_mp = load_pesos()
    link_article = look_article_img(text)
    repeated_imgs = count_repeated_img(pesos_rb_cls, pesos_rb_mp, pesos_st_cls, pesos_st_mp)
    fig = display_images(repeated_imgs, link_article)
